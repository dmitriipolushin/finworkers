...
def mixEsmEgp():
    for ActualGamer in range(player.CurNumOfGamers-1): #процедура сортировки заявок 
        for i in range(player.CurNumOfGamers-ActualGamer):
    ...
    for ActualGamer in range(player.CurNumOfGamers):
        if (quantity[ActualGamer]==0):
            ...
            print(ActualGamer+1,'. ','Заявка игрока ',numbersmix[ActualGamer],' величиной ',value[ActualGamer])
def satisfactionEsm(Lvl, rmngESM): #процедура удовлетворения заявок
    for ActualGamer in range(player.CurNumOfGamers):
        if ((rmngESM>=quantity[ActualGamer]) and (rmngESM!=0)): #проверка поданой заявки. Если продается есм больше, чем покупается
            player.players[numbersmix[ActualGamer]]['money']-=value[ActualGamer]*quantity[ActualGamer]
            player.players[numbersmix[ActualGamer]]['esm']+= quantity[ActualGamer]
            ...
            player.players[numbersmix[ActualGamer]]['money']-=value[ActualGamer]*rmngESM
            player.players[numbersmix[ActualGamer]]['esm']+= rmngESM
            print(' Игрок ',ActualGamer+1,' получает ',quantity[ActualGamer],' ЕСМ')
        else: # если есм не соталось
            print(' Все ЕСМ были раскуплены ')
            print('оставшиеся деньги игрока ',numbersmix[ActualGamer],' = ',player.players[numbersmix[ActualGamer]]['money'])
def satisfactionEgp(Lvl,rmngEGP): #процедура удовлетворения заявок егп
    for ActualGamer in range(player.CurNumOfGamers):
        if ((rmngEGP>=quantity[ActualGamer]) and (rmngEGP!=0)): #проверка поданой заявки. Если прокупается егп больше, чем продается
            print(ActualGamer,'. ','Заявка игрока ',numbersmix[ActualGamer],' величиной ',value[ActualGamer],' за одну ЕГП')
            print('Прибыль игрока ',ActualGamer,' = ',value[ActualGamer]*quantity[ActualGamer])
            player.players[numbersmix[ActualGamer]]['money']+=value[ActualGamer]*quantity[ActualGamer]
            player.players[numbersmix[ActualGamer]]['egp']-= quantity[ActualGamer]
            ...
            player.players[numbersmix[ActualGamer]]['money']-=value[ActualGamer]*rmngEGP
            player.players[numbersmix[ActualGamer]]['egp']+= rmngEGP
            print('Прибыль игрока ',ActualGamer,' = ',value[ActualGamer]*rmngEGP)
        else: # если было скупленнно нужное кол-во егп
            print('   Заказчик закупил максимальное кол-во ЕГП ')
            print(' деньги игрока ',numbersmix[ActualGamer],' c учетом прибыли = ',player.players[numbersmix[ActualGamer]]['money'])