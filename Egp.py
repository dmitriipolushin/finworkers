...
def AcquisitionOfEgp(Lvl): #процедура для продажи егп игроками
    ...
    for ActualGamer in range(player.CurNumOfGamers):
        ...
        print('Общее кол-во скупаемых ЕГП = ',tabl.a[Lvl-1][2],', МаксЦенаЕГП = ',tabl.a[Lvl][3])
        print('Игрок ',ActualGamer+1)
        print('Ваши деньги= ',player.qplayers[ActualGamer]['money'])
        print('Сколько ЕГП вы хотите продать? ')
        qntEGP=int(input())
        while(qntEGP>tabl.a[Lvl-1][2]):
            print('Недопустимое кол-во. Вы можете продать не более ',tabl.a[Lvl][2],' ЕГП ')
            print('Введите другое кол-во ')
            qntEGP=int(input())
        if (qntEGP!=0):
            print('По какой цене? ')
            prEGP=int(input())
            while (qntEGP>tabl.a[Lvl-1][2]) or (prEGP>tabl.a[Lvl-1][3]): #Здесь он проверяет все ли в порядке с заявкой
